...
import { 
  FaRupeeSign, FaReceipt, FaDownload, FaEye, FaSearch, 
  FaMoneyBillWave, FaCalendarAlt, FaCheckCircle, FaTimesCircle, 
  FaUser, FaClock, FaFileInvoice, FaWallet, FaChartLine,
...
const FeeManagement = () => {
  ...
  const handleCourseSelect = (courseId) => {
    ...
    const admissionFeeAmount = createFeeData.admissionFee || 0
    const totalWithAdmission = course.totalFees + admissionFeeAmount
    ...
    if (course.duration?.unit === 'months') {
      maxInstallments = Math.min(course.duration.value, 12)
    } else if (course.duration?.unit === 'years') {
      maxInstallments = Math.min(course.duration.value * 12, 12)
    } else {
      maxInstallments = Math.min(Math.ceil(course.duration.value / 4), 6)
    }
    ...
    setCreateFeeData(prev => ({
      ...
      numberOfInstallments: Math.min(prev.numberOfInstallments || 1, maxInstallments),
      customAmount: finalAmount
    }))
    
    if (createFeeData.dueDate) {
      generateInstallments(finalAmount, createFeeData.numberOfInstallments || 1, createFeeData.dueDate)
    }
  }
  ...
  const generateInstallments = (totalAmount, numberOfInstallments, dueDate) => {
    if (!totalAmount || totalAmount <= 0 || !numberOfInstallments || numberOfInstallments < 1) {
      setInstallments([])
      return
    }
  ...
  const handleDiscountChange = (type, value) => {
    ...
    setCreateFeeData(prev => ({
      ...prev,
      discount: { type, value: discountValue, amount: discountAmount },
      customAmount: finalAmount
    }))
    
    if (createFeeData.totalFees > 0) {
      generateInstallments(finalAmount, createFeeData.numberOfInstallments, createFeeData.dueDate)
    }
  }
  ...
  const handleInstallmentCountChange = (count) => {
    ...
    const newCount = Math.min(parseInt(count) || 1, maxInstallments)
    setCreateFeeData(prev => ({ ...prev, numberOfInstallments: newCount }))
  ...
  const handleCreateFee = async (e) => {
    ...
    try {
      ...
      const response = await feeService.createFeeRecord(feeData)
      if (response.success) {
    ...
    <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-3">
          <Button onClick={() => setShowCreateFeeModal(true)} className="bg-blue-600 hover:bg-blue-700">
      ...
      {/* Alerts */}
      {error && <Alert type="error" message={error} onClose={() => setError('')} className="animate-fade-in" />}
      {success && <Alert type="success" message={success} onClose={() => setSuccess('')} className="animate-fade-in" />}
      ...
      <Card className="p-6 transition-all duration-300">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <Button variant="secondary" onClick={handleClearFilters} className="flex items-center justify-center gap-2">
              ...
              <tr key={fee._id} className="hover:bg-gray-50 transition-all duration-200">
                ...
                <td className="px-4 py-3">
                  <div className="flex justify-center gap-2">
                    <button onClick={() => handleViewReceipt(fee)} className="text-blue-600 hover:text-blue-800" title="View Receipt">
                      <FaReceipt />
                    </button>
                    {fee.status !== 'paid' && (
                      <button onClick={() => {
                    ...
                    <button onClick={() => handleSendReminder(fee)} className="text-yellow-600 hover:text-yellow-800" title="Send Reminder">
                      <FaEnvelope />
                    </button>
                  </div>
                </td>
      ...
      <Modal isOpen={showCreateFeeModal} onClose={() => setShowCreateFeeModal(false)} title="Create Fee Structure" size="lg">
        <form onSubmit={handleCreateFee} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Admission Fee (₹) - Same for all courses</label>
            <Input
              type="number"
            ...
            <>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => handleInstallmentCountChange(createFeeData.numberOfInstallments - 1)}
                    disabled={createFeeData.numberOfInstallments <= 1} className="p-2 border rounded-lg disabled:opacity-50 hover:bg-gray-50">
                    <FaMinus />
                  </button>
                  <input type="number" value={createFeeData.numberOfInstallments}
                    onChange={(e) => handleInstallmentCountChange(e.target.value)}
                    min="1" max={selectedCourse.duration?.unit === 'months' ? Math.min(selectedCourse.duration.value, 12) : 6}
                  ...
                  <div className="border rounded-lg overflow-hidden max-h-48 overflow-y-auto">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 sticky top-0">
                        <tr><th className="px-3 py-2 text-left">Installment</th><th className="px-3 py-2 text-left">Amount</th><th className="px-3 py-2 text-left">Due Date</th></tr>
                      </thead>
                      <tbody>
              ...
              <Input label="First Installment Due Date *" type="date" value={createFeeData.dueDate}
                onChange={(e) => {
                  setCreateFeeData({ ...createFeeData, dueDate: e.target.value })
                  if (selectedCourse) generateInstallments(createFeeData.customAmount, createFeeData.numberOfInstallments, e.target.value)
                }} required />
            </>
          )}

          <div className="flex justify-end gap-3 pt-4">
            <Button type="button" variant="secondary" onClick={() => { setShowCreateFeeModal(false); resetCreateFeeForm() }}>Cancel</Button>
            <Button type="submit" isLoading={loading} disabled={!createFeeData.courseId}>Create Fee Structure</Button>
          </div>
        </form>
      </Modal>
      ...
      <Modal isOpen={showPaymentModal} onClose={() => setShowPaymentModal(false)} title="Collect Payment" size="md">
        <form onSubmit={handlePayment} className="space-y-4">
          {selectedStudent && (
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div><p className="font-medium text-gray-900">{selectedStudent.name}</p><p className="text-sm text-gray-500">{selectedStudent.enrollmentNo}</p></div>
              </div>
            </div>
          )}
          <Input label="Amount (₹)" type="number" value={paymentData.amount} onChange={(e) => setPaymentData({ ...paymentData, amount: e.target.value })} required min="1" />
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Payment Mode</label>
            <select value={paymentData.paymentMode} onChange={(e) => setPaymentData({ ...paymentData, paymentMode: e.target.value })} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <Input label="Payment Date" type="date" value={paymentData.paymentDate} onChange={(e) => setPaymentData({ ...paymentData, paymentDate: e.target.value })} required />
          <Input label="Transaction ID (Optional)" value={paymentData.transactionId} onChange={(e) => setPaymentData({ ...paymentData, transactionId: e.target.value })} placeholder="Enter transaction reference" />
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Notes</label><textarea value={paymentData.notes} onChange={(e) => setPaymentData({ ...paymentData, notes: e.target.value })} rows="2" className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
          <div className="flex justify-end gap-3 pt-4"><Button type="button" variant="secondary" onClick={() => setShowPaymentModal(false)}>Cancel</Button><Button type="submit" isLoading={loading}>Process Payment</Button></div>
        </form>
      </Modal>
      ...
      <Modal isOpen={showReceiptModal} onClose={() => setShowReceiptModal(false)} title="Payment Receipt" size="md">
        {selectedFee && (
          <div className="space-y-4">
            <div className="text-center border-b pb-4"><FaFileInvoice className="text-4xl text-blue-600 mx-auto mb-2" /><h2 className="text-xl font-bold">Payment Receipt</h2><p className="text-sm text-gray-500">Receipt #{selectedFee._id?.slice(-6)}</p></div>
            <div className="space-y-2">
              <div className="flex justify-between"><span className="text-gray-600">Student Name:</span><span className="font-medium">{selectedFee.studentId?.name}</span></div>
              <div className="flex justify-between"><span className="text-gray-600">Enrollment No:</span><span className="font-medium">{selectedFee.studentId?.enrollmentNo}</span></div>
              <div className="flex justify-between"><span className="text-gray-600">Course:</span><span className="font-medium">{selectedFee.courseId?.name}</span></div>
              <div className="flex justify-between"><span className="text-gray-600">Total Fees:</span><span className="font-medium">₹{selectedFee.totalFees?.toLocaleString()}</span></div>
              <div className="flex justify-between"><span className="text-gray-600">Amount Paid:</span><span className="font-medium text-green-600">₹{selectedFee.paidAmount?.toLocaleString()}</span></div>
              <div className="flex justify-between"><span className="text-gray-600">Pending Balance:</span><span className="font-medium text-red-600">₹{selectedFee.pendingAmount?.toLocaleString()}</span></div>
              <div className="flex justify-between border-t pt-2 mt-2"><span className="text-gray-600">Payment Status:</span><span className={`font-medium ${getStatusColor(selectedFee.status)}`}>{getStatusText(selectedFee.status)}</span></div>
            </div>
            <div className="flex gap-3 pt-4"><Button variant="secondary" onClick={() => setShowReceiptModal(false)} className="flex-1">Close</Button><Button onClick={() => window.print()} className="flex-1"><FaPrint className="inline mr-2" />Print Receipt</Button></div>
          </div>
        )}
      </Modal>
          ...
          <div className="space-y-3">
            {selectedFee.installmentPlan.installments.map((inst, idx) => (
              <div key={idx} className="border rounded p-3">
                <div className="flex justify-between items-center"><span className="font-medium">Installment {inst.installmentNumber}</span><span className={`text-xs px-2 py-1 rounded ${inst.status === 'paid' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{inst.status}</span></div>
                <div className="grid grid-cols-2 gap-2 mt-2 text-sm">
                  <div><span className="text-gray-600">Amount:</span><span className="ml-2 font-medium">₹{inst.amount.toLocaleString()}</span></div>
                  <div><span className="text-gray-600">Paid:</span><span className="ml-2">₹{inst.paidAmount?.toLocaleString() || 0}</span></div>
                  <div><span className="text-gray-600">Due Date:</span><span className="ml-2">{format(new Date(inst.dueDate), 'dd MMM yyyy')}</span></div>
                  {inst.paidDate && <div><span className="text-gray-600">Paid Date:</span><span className="ml-2">{format(new Date(inst.paidDate), 'dd MMM yyyy')}</span></div>}
                </div>
              </div>
            ))}
          </div>
